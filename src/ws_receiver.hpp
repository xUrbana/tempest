...
public:
    ...
    WebsocketReceiver(const std::string &token, std::string device_id, std::shared_ptr<QueueType> queue) :
        ...
        queue_(std::move(queue))
    {
        ...
        {
            throw std::runtime_error(std::format("Invalid token or device id passed to websocket receiver. Token: {}, Device ID: {}", token, device_id));
        }
...
private:
    void init()
    {
        ...
        ws_client_.set_message_handler([this](auto handle, auto msg) { return handle_receive(handle, msg); });
